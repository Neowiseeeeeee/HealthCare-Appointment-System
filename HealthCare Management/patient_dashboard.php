 ...
 <head>
  ...
  <script>
        ...
        fetch('api/get_patient_messages.php')
          ...
          .then(data => {
            ...
            messages.forEach(message => {
              // Create message item
              const item = document.createElement('div');
              item.className = 'message-item bg-white rounded-lg p-3 cursor-pointer transition-all hover:shadow-md mb-3 border border-gray-200';
              ...
              // Format date
              let formattedDate = message.created_at;
              ...
              item.innerHTML = `
                <div class="flex">
                  <div class="flex-1">
                    <div class="flex justify-between items-start">
                      <h4 class="font-semibold text-sm">${message.sender_name || 'System'}</h4>
                      <span class="text-xs text-gray-500">${formattedDate}</span>
                    </div>
                    <p class="font-medium text-sm mt-1">${message.title || 'No subject'}</p>
                    <p class="text-sm text-gray-600 mt-1">${message.content.length > 60 ? message.content.substring(0, 60) + '...' : message.content}</p>
                  </div>
                </div>
              `;
        ...
        fetch('api/get_patient_messages.php')
          ...
          .then(data => {
            ...
            messages.forEach(message => {
              ...
              item.addEventListener('click', () => {
                // Fetch endpoint to mark message as read
                if (!message.is_read) {
                  fetch('api/mark_as_read.php', {
                    ...
                    body: `type=message&item_id=${message.message_id}`
                  })
                  .then(response => response.json())
                  .then(data => {
                    if (data.success) {
              ...
              item.innerHTML = `
                <div class="flex justify-between items-start">
                  <div>
                    <h4 class="font-semibold text-sm">${message.subject || 'No Subject'}</h4>
                    <div class="text-xs text-gray-500">From: ${message.sender_name}</div>
                    <div class="text-xs text-gray-500">${message.date || message.created_at || ''}</div>
                  </div>
                  <div class="text-gray-400">
                    <i class="fas fa-envelope${message.is_read ? '-open' : ''}"></i>
                  </div>
                </div>
                <p class="text-sm text-gray-600 mt-1">${message.content.length > 100 ? message.content.substring(0, 100) + '...' : message.content}</p>
              `;
      ...
      function showMessageModal(message) {
        // Set up modal with message content
        const modal = document.getElementById('messageModal');
        ...
        if (modal && title && content && sender && date) {
          title.textContent = message.subject || 'No Subject';
          content.textContent = message.content;
          sender.textContent = 'From: ' + message.sender_name;
          date.textContent = 'Date: ' + (message.date || message.created_at || '');
    ...
    <div class="lg:w-[30%]">
     ...
     >
      <div class="p-6 h-full flex flex-col">
       ...
       <div class="messages-filter flex space-x-2 mb-4">
         <button class="filter-btn bg-blue-600 text-white px-3 py-1 text-sm rounded active" data-filter="all" onclick="filterMessages('all')">All</button>
         <button class="filter-btn bg-gray-200 hover:bg-gray-300 px-3 py-1 text-sm rounded" data-filter="unread" onclick="filterMessages('unread')">Unread</button>
       </div>
       <div id="messagesList" class="space-y-3 overflow-y-auto flex-1 pr-2">
